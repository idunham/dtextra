...
/* bug fixed and built-in types "directory" and "emptydir" added by
   Albert Graef, Mar 10 1996 */
...
static char *builtin_test()
{
	...
	while ((entry = readdir(dir)) &&
	    ...
	    ;
	closedir(dir);
	return builtin_result = entry?"directory":"emptydir";
    }
    if(bytes > 0)
    {
	if(bytes > ASCLEN)
