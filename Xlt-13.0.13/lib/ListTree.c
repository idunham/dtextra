/*
 *  $Id: ListTree.c,v 1.20 2001/08/27 14:30:42 amai Exp $
 */
/*-----------------------------------------------------------------------------
...
/* Check out ListTree.h! */
#define	LISTTREE_COMPAT
...
#ifdef DEBUG
static void DBG(int line, const char *fcn, const char *fmt, ...)
{
  va_list ap;
...
#define offset(field) XtOffsetOf(XltListTreeRec, list.field)
static XtResource resources[] =
{
  ...
  {XltNdefaultMode, XltCDefaultMode, XtRBoolean, sizeof(Boolean),
...
static void Destroy(XltListTreeWidget w);
static void Redisplay(Widget aw, XExposeEvent * event, Region region);
static void Resize(XltListTreeWidget w);
...
static XtGeometryResult QueryGeometry(XltListTreeWidget w, XtWidgetGeometry *proposed, XtWidgetGeometry *answer);

static void Draw(XltListTreeWidget w, int yevent,int hevent);
static void DrawAll(XltListTreeWidget w);
static void DrawChanged(XltListTreeWidget w);
static void DrawItemHighlight(XltListTreeWidget w, XltListTreeItem *item);
static void DrawItemHighlightClear(XltListTreeWidget w, XltListTreeItem *item);
static XltListTreeItem *GetItem(XltListTreeWidget w, int findy);

static void DeleteChildren(XltListTreeWidget w, XltListTreeItem *item);
static void CountAll(XltListTreeWidget w);
static void GotoPosition(XltListTreeWidget w);
static int  GotoPositionChildren(XltListTreeWidget w,XltListTreeItem *item, int i);
...
static int SearchPosition(XltListTreeWidget w, XltListTreeItem *item, XltListTreeItem *finditem,
	       int y, Boolean *found);
...
static XmBaseClassExtRec xltListtreeCoreClassExtRec =
{
    /* next_extension            */ NULL,
...
static XmPrimitiveClassExtRec xltListtreePrimClassExtRec =
{
    /* next_extension      */ NULL,
    /* record_type         */ NULLQUARK,
...
XltListTreeClassRec xltListtreeClassRec =
{
  {
	/* core_class fields     */
	/* MOTIF superclass      */ (WidgetClass) & xmPrimitiveClassRec,
	/* class_name            */ "XltListTree",
	/* widget_size           */ sizeof(XltListTreeRec),
	...
	/* extension             */ (XtPointer) & xltListtreeCoreClassExtRec
  },
    /* Primitive Class part */
  {
	...
	/* num syn res           */ 0,
	/* extension             */ (XtPointer) & xltListtreePrimClassExtRec,
  },
  {
...
WidgetClass xltListtreeWidgetClass = (WidgetClass) & xltListtreeClassRec;

static void
MakePixmap(XltListTreeWidget w, Pixinfo * pix)
{
  Window root;
  int x, y;
...
static void
FreePixmap(XltListTreeWidget w, Pixinfo * pix)
{
  if (pix->pix)
...
static void
InitializePixmaps(XltListTreeWidget w)
{
...
static void
InitializeGC(XltListTreeWidget w)
{
  XGCValues values;
  XtGCMask mask;
...
static void
InitializeScrollBars(XltListTreeWidget w)
{
...
static void
InitializeGeometry(XltListTreeWidget w)
{
  w->list.XOffset = 0;
...
static void
Destroy(XltListTreeWidget w)
{
  XltListTreeItem *item, *sibling;
...
{
  XltListTreeWidget w = (XltListTreeWidget) aw;
...
Boolean refresh = False;
  ...
  if (!XtIsRealized(current))
    ...
    if (((XltListTreeWidget)current)->list.Indent != ((XltListTreeWidget)reply)->list.Indent)
    {
    	relayout = True;
    }
    if (relayout)
...
static void
ResizeStuff(XltListTreeWidget w)
{
  XRectangle clip;
...
static void
SetScrollbars(XltListTreeWidget w)
{
  if (w->list.vsb) {
...
{
  XltListTreeWidget w = (XltListTreeWidget) client_data;
...
QueryGeometry(XltListTreeWidget w, XtWidgetGeometry *proposed, XtWidgetGeometry *answer)
{
...
#ifdef DEBUG_TREE
void
ItemCheck(XltListTreeWidget w, XltListTreeItem * item)
{
  XltListTreeItem *p;
  char text[1024];
...
void
ChildrenCheck(XltListTreeWidget w, XltListTreeItem * item)
{
  while (item) {
    ItemCheck(w, item);
    if (item->firstchild)
...
void
TreeCheck(XltListTreeWidget w, char *txt)
{
...
static void
HighlightItem(XltListTreeWidget w, XltListTreeItem * item, Boolean state, Boolean draw)
{
  if (item) {
...
static void
HighlightChildren(XltListTreeWidget w, XltListTreeItem * item, Boolean state, Boolean draw)
{
  while (item) {
...
static void
HighlightAll(XltListTreeWidget w, Boolean state, Boolean draw)
{
  HighlightChildren(w,w->list.first,state,draw);
}

static void
HighlightVisibleChildren(XltListTreeWidget w, XltListTreeItem * item, Boolean state, Boolean draw)
{
  while (item) {
    HighlightItem(w, item, state, draw);
...
static void
HighlightAllVisible(XltListTreeWidget w, Boolean state, Boolean draw)
{
  XltListTreeItem *item;
...
static void
AddItemToReturnList(XltListTreeWidget w, XltListTreeItem * item, int loc)
{
  if (loc >= w->list.ret_item_alloc) {
    w->list.ret_item_alloc += ListTreeRET_ALLOC;
    w->list.ret_item_list = (XltListTreeItem **) XtRealloc((char *) w->list.ret_item_list, w->list.ret_item_alloc * sizeof(XltListTreeItem *));
  }
  w->list.ret_item_list[loc] = item;
}

static void
MultiAddToReturn(XltListTreeWidget w, XltListTreeItem * item, ListTreeMultiReturnStruct * ret)
{
...
static void
HighlightCount(XltListTreeWidget w, XltListTreeItem * item, ListTreeMultiReturnStruct * ret)
{
  while (item) {
    if (item->highlighted)
...
static void
MakeMultiCallbackStruct(XltListTreeWidget w, ListTreeMultiReturnStruct * ret)
{
  XltListTreeItem *item;
...
static void
HighlightDoCallback(XltListTreeWidget w)
{
...
static void
MakeActivateCallbackStruct(XltListTreeWidget w, XltListTreeItem * item, ListTreeActivateStruct * ret)
{
  int count;
  XltListTreeItem *parent;
...
static void
SelectDouble(XltListTreeWidget w)
{
  ...
  if (w->list.timer_item) {
    ...
    if (w->list.HighlightPath) {
      ...
      XltListTreeSetHighlighted((Widget)w,ret.path,ret.count,True);
      w->list.Refresh=save;
/*       ListTreeGetHighlighted(w,&ret2); */
/*       XltListTreeSetHighlighted(w,ret2.items,ret2.count,True); */
    }
...
menu(Widget aw, XEvent *event, String *params, Cardinal *num_params)
{
  XltListTreeWidget w = (XltListTreeWidget) aw;
  XltListTreeItem *item;
  XltListTreeItemReturnStruct ret;
...
static Pixinfo *
GetItemPix(XltListTreeWidget w, XltListTreeItem *item)
{
  Pixinfo *pix;
...
static void
DrawItemHighlight(XltListTreeWidget w, XltListTreeItem *item)
{
  int width;
...
static void
DrawItemHighlightClear(XltListTreeWidget w, XltListTreeItem *item)
{
  int width;
...
static void
DrawItem(XltListTreeWidget w, XltListTreeItem *item,
...
static int
DrawChildren(XltListTreeWidget w, XltListTreeItem *item, XltListTreeItem **last,
  int y, int xroot, int yroot)
{
  int width, height;
  int xbranch, ybranch;
 ...
 */
static void
DrawVertical(XltListTreeWidget w, XltListTreeItem *item)
{
  int xroot,yroot, pos;
...
static void
Draw(XltListTreeWidget w, int yevent,int hevent)
{
  int y, xbranch, ybranch;
  XltListTreeItem *item,*lastdrawn;
...
static void
DrawChanged(XltListTreeWidget w)
{
  DrawAll(w);
  SetScrollbars(w);
}
...
/* Counting functions ------------------------------------------------------- */
static int
GotoPositionChildren(XltListTreeWidget w,XltListTreeItem *item, int i)
{
...
static void
GotoPosition(XltListTreeWidget w)
{
...
static int
CountItem(XltListTreeWidget w, XltListTreeItem *item, int x, int y)
{
...
static int
CountChildren(XltListTreeWidget w, XltListTreeItem *item, int x, int y)
{
  int height;
...
static void
RemoveReference(XltListTreeWidget w, XltListTreeItem *item)
{
...
static void
DeleteChildren(XltListTreeWidget w, XltListTreeItem *item)
{
  XltListTreeItem *sibling;
  XltListTreeItemReturnStruct ret;
...
static void
InsertChild(XltListTreeWidget w, XltListTreeItem *parent, XltListTreeItem *item)
{
  XltListTreeItem *i;
...
static void
InsertChildren(XltListTreeWidget w, XltListTreeItem *parent, XltListTreeItem *item)
{
  XltListTreeItem *next, *newnext;
...
static int
SearchChildren(XltListTreeWidget w, XltListTreeItem *item, XltListTreeItem **last,
  int y, int findy, XltListTreeItem **finditem)
{
  while (item) {
...
static XltListTreeItem *
GetItem(XltListTreeWidget w, int findy)
{
  int y;
  XltListTreeItem *item, *finditem, *lastdrawn;
...
static int
SearchPosition(XltListTreeWidget w, XltListTreeItem *item, XltListTreeItem *finditem,
	       int y, Boolean *found)
{
  int height;
  Pixinfo *pix;
...
static Position
GetPosition(XltListTreeWidget w, XltListTreeItem *finditem)
{
  int y, height;
  XltListTreeItem *item;
  Pixinfo *pix;
...
void
XltListTreeRefresh(Widget w)
{
  if (XtIsRealized((Widget) w) && ((XltListTreeWidget)w)->list.Refresh)
    DrawChanged((XltListTreeWidget)w);
}


void
XltListTreeRefreshOff(Widget w)
{
  ((XltListTreeWidget)w)->list.Refresh = False;
}


void
XltListTreeRefreshOn(Widget w)
{
  ((XltListTreeWidget)w)->list.Refresh = True;
  XltListTreeRefresh(w);
}


static XltListTreeItem *
AddItem(XltListTreeWidget w, XltListTreeItem * parent, char *string,
  XltListTreeItemType type)
{
  ...
  len = strlen(string);
  item = (XltListTreeItem *) XtMalloc(sizeof(XltListTreeItem));
...
XltListTreeItem *
XltListTreeAdd(Widget w,XltListTreeItem *parent,char *string)
{
    return (AddItem ((XltListTreeWidget)w,parent,string,ItemDetermineType));
}


XltListTreeItem *
XltListTreeAddType(Widget w, XltListTreeItem *parent, char *string,
                XltListTreeItemType type)
{
    return (AddItem( (XltListTreeWidget)w, parent, string, type));
}


XltListTreeItem *
XltListTreeAddBranch(Widget w,XltListTreeItem *parent,char *string)
{
    return (AddItem( (XltListTreeWidget)w,parent,string,ItemBranchType));
}


XltListTreeItem *
XltListTreeAddLeaf(Widget w,XltListTreeItem *parent,char *string)
{
    return (AddItem( (XltListTreeWidget)w,parent,string,ItemLeafType));
}


void
XltListTreeSetItemPixmaps (Widget w, XltListTreeItem *item,
...
void
XltListTreeRenameItem(Widget w, XltListTreeItem * item, char *string)
{
  int len;
  char *copy;
...
int
XltListTreeDelete(Widget w, XltListTreeItem * item)
{
  if (item->firstchild)
    DeleteChildren((XltListTreeWidget)w, item->firstchild);
  item->firstchild = NULL;

  RemoveReference((XltListTreeWidget)w, item);
...
int
XltListTreeDeleteChildren(Widget w, XltListTreeItem * item)
{
  if (item->firstchild)
    DeleteChildren((XltListTreeWidget)w, item->firstchild);
  item->firstchild = NULL;
...
int
XltListTreeReparent(Widget w, XltListTreeItem * item, XltListTreeItem * newparent)
{
...
/* The item is now unattached.  Reparent it.                     */
  InsertChild((XltListTreeWidget)w, newparent, item);
...
int
XltListTreeReparentChildren(Widget w, XltListTreeItem * item, XltListTreeItem * newparent)
{
  XltListTreeItem *first;
  ...
  if (item->firstchild) {
    ...
    InsertChildren((XltListTreeWidget)w, newparent, first);
...
int
XltListTreeUserOrderSiblings(Widget w, XltListTreeItem * item, int (*func) (const void *, const void *))
{
  XltListTreeItem *first, *parent, **list;
  size_t i, count, size;

  TreeCheck(w, "in XltListTreeUserOrderSiblings");
...
/* Count the children */
  ...
  size = sizeof(XltListTreeItem *);
  list = (XltListTreeItem **) XtMalloc(size * count);
  list[0] = first;
  count = 1;
  ...
  if (parent)
    ...
    ((XltListTreeWidget)w)->list.first = list[0];
  XtFree((char *) list);

  XltListTreeRefresh(w);
  TreeCheck(w, "exiting XltListTreeOrderSiblings");
...
int
XltListTreeOrderSiblings(Widget w, XltListTreeItem * item)
{
  TreeCheck(w, "in XltListTreeOrderSiblings");
  return XltListTreeUserOrderSiblings(w, item, AlphabetizeItems);
}


int
XltListTreeUserOrderChildren(Widget w, XltListTreeItem * item, int (*func) (const void *, const void *))
{
  XltListTreeItem *first;
  ...
  if (item) {
    first = item->firstchild;
    if (first)
      XltListTreeUserOrderSiblings(w, first, func);
  }
  else {
    if (((XltListTreeWidget)w)->list.first)
      XltListTreeUserOrderSiblings(w, ((XltListTreeWidget)w)->list.first, func);
  }
...
int
XltListTreeOrderChildren(Widget w, XltListTreeItem * item)
{
  XltListTreeItem *first;
  ...
  if (item) {
    first = item->firstchild;
    if (first)
      XltListTreeOrderSiblings(w, first);
  }
  else {
    if (((XltListTreeWidget)w)->list.first)
      XltListTreeOrderSiblings(w, ((XltListTreeWidget)w)->list.first);
  }
...
XltListTreeItem *
XltListTreeFindSiblingName(Widget w, XltListTreeItem * item, char *name)
{
  XltListTreeItem *first;
...
XltListTreeItem *
XltListTreeFindChildName(Widget w, XltListTreeItem * item, char *name)
{
...
/* Get first child in list; */
  ...
  else if (!item && ((XltListTreeWidget)w)->list.first) {
    item = ((XltListTreeWidget)w)->list.first;
  }
  else
    item = NULL;
...
void
XltListTreeHighlightItem(Widget w, XltListTreeItem * item)
{
  HighlightAll((XltListTreeWidget)w, False, False);
  HighlightItem((XltListTreeWidget)w, item, True, False);
  XltListTreeRefresh(w);
}


void
XltListTreeHighlightAll(Widget w)
{
  HighlightAllVisible((XltListTreeWidget)w, True, False);
  XltListTreeRefresh(w);
}


void
XltListTreeClearHighlighted(Widget w)
{
  HighlightAll((XltListTreeWidget)w, False, False);
  XltListTreeRefresh(w);
}
...
void
XltListTreeGetHighlighted(Widget w, ListTreeMultiReturnStruct * ret)
{
  if (ret)
    MakeMultiCallbackStruct((XltListTreeWidget)w, ret);
}


void
XltListTreeSetHighlighted(Widget w, XltListTreeItem ** items, int count, Boolean clear)
{
  if (clear)
    HighlightAll((XltListTreeWidget)w, False, False);
  if (count < 0) {
    while (*items) {
      HighlightItem((XltListTreeWidget)w, *items, True, False);
      items++;
    }
  }
  else {
    ...
    for (i = 0; i < count; i++) {
      HighlightItem((XltListTreeWidget)w, items[i], True, False);
    }
  }
  XltListTreeRefresh(w);
}
...
XltListTreeItem *
XltListTreeFirstItem(Widget w)
{
  XltListTreeItem *first;

/* Get first child in widget */
  first = ((XltListTreeWidget)w)->list.first;
  return first;
}


Position
XltListTreeGetItemPosition(XltListTreeWidget w, XltListTreeItem * item)
{
...
void
XltListTreeGetPathname(ListTreeReturnStruct * ret, char *dir)
{
  int count;
...
void
XltListTreeGetPathnameFromItem(XltListTreeItem * item, char *dir)
{
  char tmppath[1024];
...
Widget
XltCreateScrolledListTree(Widget parent, char *name, Arg *args, Cardinal count)
{
  Widget sw;
  char *sname;
  Cardinal i;
  ...
  return XtCreateWidget(name, xltListtreeWidgetClass, sw, args, count);
}
...
{
	return XltCreateScrolledListTree(parent, name, args, count);
}

void
XltListTreeOpen(Widget w, XltListTreeItem *item, Boolean s)
{
        if (item)
                item->open = s;
