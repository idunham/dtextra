/* $Id: SciPlot.c,v 1.17 2001/08/27 14:30:42 amai Exp $
 * ----------------------------------------------------------------------------
 *
 ...
 * 
 * Changes: added double buffering (Andrei Romanenko <andrew@eq.uc.pt>)
 *          added basic UIL support (Andrei Romanenko <andrew@eq.uc.pt>)         
 *
 */
...
static void Redisplay(SciPlotWidget w, XExposeEvent *event, Region region);
static void Resize(SciPlotWidget w);
static Boolean SetValues(SciPlotWidget current, SciPlotWidget request,
                         SciPlotWidget new, ArgList args, Cardinal nargs);
static void GetValuesHook(SciPlotWidget w, ArgList args, Cardinal *num_args);
...
{
  FILE *fd;
  float scale, xoff, yoff, xmax, ymax, yflip, aspect, titlefontsize;
  float border=0.;
  int i;
  PScommands *p;
  char fontname[128];
...
Widget
XltCreateSciPlot(Widget parent, char *name, Arg *al, Cardinal ac)
{
	return XtCreateWidget(name, sciplotWidgetClass,
		parent, al, ac);
}
...
Widget
SciPlotCreate(Widget parent, char *name, Arg *arglist, int nargs)
{
...
#if 0
Cardinal
SciPlotMrmInitialize(void)
{
    Cardinal mrc;

    mrc = MrmRegisterClass (MrmwcUnknown, "SciPlot" , "SciPlotCreate",
